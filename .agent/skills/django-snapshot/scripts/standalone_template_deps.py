...
def _print_tree(templates: dict[str, dict], reverse_map: dict[str, list[str]]) -> None:
    ...
    def _print_node(name: str, depth: int, visited: set[str]) -> None:
        ...
        blocks = (
            f"  [{', '.join(info.get('blocks', [])[:3])}]" if info.get("blocks") else ""
        )
        print(f"{indent}{marker}{name}{blocks}")
        if name in visited:
            return
        visited.add(name)
        children = sorted(n for n, i in templates.items() if i.get("extends") == name)
...
def _print_summary(
    templates: dict[str, dict], reverse_map: dict[str, list[str]]
) -> None:
    total = len(templates)
