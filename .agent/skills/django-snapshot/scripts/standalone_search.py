...
def _iter_files(directories: list[Path], include_pattern: str) -> Iterator[Path]:
    ...
    for d in directories:
        for f in d.rglob(include_pattern):
            ...
            if any(
                p in parts
                for p in (".venv", "__pycache__", "node_modules", ".git", "migrations")
            ):
                continue
            yield f
...
範例：
  ...
  python standalone_search.py "{% url" --include "*.html" --case-sensitive
    ...
    parser.add_argument(
        "--regex", action="store_true", help="將 pattern 視為正則表達式"
    )
    parser.add_argument(
        "--include", default="*", help="檔案 glob 篩選（例：*.py、*.html）"
    )
    parser.add_argument("--app", help="只搜尋指定 app 目錄")
    parser.add_argument(
        "--context", type=int, default=0, metavar="N", help="顯示前後 N 行上下文"
    )
    parser.add_argument(
