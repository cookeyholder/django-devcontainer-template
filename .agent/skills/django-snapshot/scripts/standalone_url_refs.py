...
def _find_project_root(start: Path) -> Path:
    for candidate in [start, *start.parents]:
        if (candidate / "manage.py").exists() or (
            candidate / "src" / "manage.py"
        ).exists():
            return candidate
    return start
        ...
        for i, line in enumerate(content.splitlines(), 1):
            for m in _TEMPLATE_URL_RE.finditer(line):
                ...
                refs.setdefault(name, []).append(
                    {"file": str(f), "lineno": i, "line": line.strip()}
                )
    return refs
...
範例：
  ...
  python standalone_url_refs.py          # 顯示所有 URL 引用摘要
    ...
    parser.add_argument(
        "-s", "--snapshots", help="snapshot_urls.json 所在目錄（預設自動偵測）"
    )
    ...
    parser.add_argument(
        "--unused", action="store_true", help="列出未被引用的 URL names"
    )
    ...
    if snapshot_dir is None:
        print(
            "錯誤: 找不到 snapshot_urls.json，請先執行 standalone_snapshot.py 生成快照",
            file=sys.stderr,
        )
        sys.exit(1)
