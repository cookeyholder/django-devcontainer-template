...
class StandaloneSnapshotGenerator:
    ...
    def _get_app_dirs(self) -> list[Path]:
        ...
        for apps_py in self.src_dir.rglob("apps.py"):
            ...
            if any(
                p in parts
                for p in (".venv", "__pycache__", "migrations", "node_modules")
            ):
                continue
    ...
    def _scan_apps(self) -> None:
        ...
        for app_dir in self._get_app_dirs():
            ...
            self.apps_data[app_name] = {
                ...
                or bool(list(app_dir.glob("*_views.py"))),
    ...
    def _infer_permissions(
        self, base_names: list[str], decorators: list[str]
    ) -> list[str]:
    ...
    def _extract_url_pattern(self, node: ast.expr, namespace: str) -> dict | None:
        ...
        key = (
            (f"{namespace}:{url_name}" if namespace else url_name)
            if url_name
            else route
        )
    ...
    def _scan_imports(self) -> None:
        ...
        for app_dir in self._get_app_dirs():
            ...
            for py_file in app_dir.rglob("*.py"):
                ...
                if imports:
                    rel = (
                        str(py_file.relative_to(app_dir))
                        .replace("/", ".")
                        .replace(".py", "")
                    )
                    app_imports[rel] = {
            ...
            for model_name in app_models:
                ...
                for other_app, other_models in self.models_data.items():
                    for other_model, other_info in other_models.items():
                        for field_name, field_info in other_info.get(
                            "fields", {}
                        ).items():
    ...
    def _generate_index(self) -> None:
        ...
        index = {
            ...
            "snapshots": {
                "apps": {
                    "file": "snapshot_apps.json",
                    "description": "Django Apps 設定",
                    "count": len(self.apps_data),
                },
                "models": {
                    "file": "snapshot_models.json",
                    "description": "Django Models 資料模型",
                    "count": models_count,
                },
                "views": {
                    "file": "snapshot_views.json",
                    "description": "Django Views 視圖函式",
                    "count": views_count,
                },
                "urls": {
                    "file": "snapshot_urls.json",
                    "description": "Django URLs 路由配置",
                    "count": len(self.urls_data),
                },
                "templates": {
                    "file": "snapshot_templates.json",
                    "description": "Django Templates 模板檔案",
                    "count": len(self.templates_data),
                },
                "forms": {
                    "file": "snapshot_forms.json",
                    "description": "Django Forms 表單",
                    "count": forms_count,
                },
                "imports": {
                    "file": "snapshot_imports.json",
                    "description": "Python Import 依賴關係",
                    "count": imports_count,
                },
                "static_assets": {
                    "file": "snapshot_static_assets.json",
                    "description": "靜態資源使用情況",
                    "count": static_count,
                },
                "cross_references": {
                    "file": "snapshot_cross_references.json",
                    "description": "跨檔案引用關係",
                    "count": cross_count,
                },
            },
